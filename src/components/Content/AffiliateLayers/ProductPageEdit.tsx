...
function ProductPage() {
  ...
  useEffect(() => {
    fetchProducts().then(productlist => {
      ...
      const id = decodeURI(window.location.pathname.substring(indexSlash + 1 + 'proizvod/edit'.length - 1));
      ...
      product.slike.split(',').map(async function (slike: any, index: any) {
        // async function rr() {
        const response = await fetch(`${baseApi}/assets/products/` + getImagePath(product as Product, index) + '.webp'); // Replace with the URL of your image
        const blob = await response.blob()
        const newFile = new File([blob], getImagePath(product as Product, index) + '.webp', { type: blob.type });
        arr.push(newFile)
        console.log(index, arr)
        // }
        // rr()
      })
      setTimeout(() => { setSelectedFile(arr) }, 1500)
      console.log(arr)
      if (product) {
        ...
        naziv_artikla_Ref.current && (naziv_artikla_Ref.current.value = product?.naziv_artikla);
        jedinica_mere_Ref.current && (jedinica_mere_Ref.current.value = product?.jedinica_mere);
        kategorija_artikla_Ref.current && (kategorija_artikla_Ref.current.value = product?.kategorija_artikla);
        transportno_pakovanje_Ref.current && (transportno_pakovanje_Ref.current.value = product?.transportno_pakovanje);
        potkategorija_Ref.current && (potkategorija_Ref.current.value = product?.potkategorija);
        minimalno_pakovanje_Ref.current && (minimalno_pakovanje_Ref.current.value = product?.minimalno_pakovanje);
        meta_description_Ref.current && (meta_description_Ref.current.value = product?.meta_description);
        kvadratura_Ref.current && (kvadratura_Ref.current.value = product?.kvadratura);
        prosireni_opis_Ref.current && (prosireni_opis_Ref.current.value = product?.prosireni_opis);
        sertifikat_Ref.current && (sertifikat_Ref.current.value = product?.sertifikat);
        ...
        tip_otpornosti_Ref.current && (tip_otpornosti_Ref.current.value = product?.tip_otpornosti);
        mesto_i_nacin_skladistenja_Ref.current && (mesto_i_nacin_skladistenja_Ref.current.value = product?.mesto_i_nacin_skladistenja);
        ...
        for (let i = 0; i < product.sifra_proizvoda.length; i++) {
          ...
          let naziv_input: any = document.getElementById(`naziv_proizvoda_model${i}`);
    ...
    try {
      ...
      if (token) {
        ...
        if (selectedFile === null || formData.get('naziv_artikla') === "/" || formData.get('count') === "0") {
          ...
          return await axios.post(`${baseApi}/products/updateProduct?id=${product._id}`, formData, header).then(res => {
          ...
          <div className='productPageProductContainer'>
            ...
            <div className='divProductContainer'>
              ...
              <div className='divProductImgSelected' ref={ref1}>
                <LazyLoadImage
                  ...
                  src={selectedFile.length && URL.createObjectURL(selectedFile[dimensionChosen])}
              ...
              <div className='small-products-images divSmallProductImagesModelViseSlika'>
                ...
                {
                  selectedFile.length && selectedFile.map((slike: string, index: any) => {
                    return (
                      <div
                        key={slike + index}
                        id={slike + index}
            ...
            <div className='productPageDescriptionContainer'>
              ...
              <div style={{ display: 'flex', width: '100%', flexWrap: 'wrap' }}>
                ...
                <div style={{ fontSize: "12pt", alignItems: 'center', width: '50%' }}>
                  <label>modelcnt*</label><input type="number" id='modelcnt' min="1" max="10" defaultValue={modelcntvalue} onChange={(e) => { cnthandlechange(Number(e.target.value)) }} />
                </div>
                ...
                <div style={{ fontSize: "12pt", alignItems: 'center', width: '50%' , display:'none'}}>
                  <label>length</label><input type="text" id='duljina' ref={duljina_Ref} />
                </div>
                <div style={{ fontSize: "12pt", alignItems: 'center', width: '50%' }}>
